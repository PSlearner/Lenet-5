...
int compare(float* feature_1, float* feature_2, int layer_name){
	int length = _msize(feature_1)/sizeof(float);
...
int main(int argc, char *argv[])
{
	...
	if(!input_layer || !hconv1_debug || !pool1_debug || !hconv2_debug || !pool2_debug || !hconv3_debug || !hfc1_debug || !output_debug){
...
//	for(int i=0;i<test_num;i++){
	for(int i=0;i<2;i++){
		for(int batch=0;batch<image_Batch*INPUT_WH*INPUT_WH;batch++){
			input_layer[batch] = MNIST_IMG[i*image_Batch*MNIST_PAD_SIZE + batch];
		}
		CONVOLUTION_LAYER_1_SW(input_layer,Wconv1,bconv1,hconv1);
//		cout << "CONVOLUTION_LAYER_1_SW :done" << endl;
//		CONVOLUTION_LAYER_1_SW_debug(input_layer,Wconv1,bconv1,hconv1_debug);
//		cout << "CONVOLUTION_LAYER_1_SW_debug :done" << endl;
//		if(!compare(hconv1, hconv1_debug, 0)){
//			break;
//		}
		
		POOLING_LAYER_1_SW(hconv1,Wpool1,bpool1,pool1);
//		cout << "POOLING_LAYER_1_SW :done" << endl;
//		POOLING_LAYER_1_SW_debug(hconv1,Wpool1,bpool1,pool1_debug);
//		cout << "POOLING_LAYER_1_SW_debug :done" << endl;
//		if(!compare(pool1, pool1_debug, 1)){
//			break;
//		}
		
		CONVOLUTION_LAYER_2_SW_debug(pool1,Wconv2,bconv2,hconv2);
//		cout << "CONVOLUTION_LAYER_2_SW :done" << endl;
//		CONVOLUTION_LAYER_2_SW_debug(pool1,Wconv2,bconv2,hconv2_debug);
//		cout << "CONVOLUTION_LAYER_2_SW_debug :done" << endl;
//		if(!compare(hconv2, hconv2_debug, 2)){
//			break;
//		}
		
		POOLING_LAYER_2_SW(hconv2,Wpool2,bpool2,pool2);
//		cout << "POOLING_LAYER_2_SW :done" << endl;
//		POOLING_LAYER_2_SW_debug(hconv2,Wpool2,bpool2,pool2_debug);
//		cout << "POOLING_LAYER_2_SW_debug :done" << endl;
//		if(!compare(pool2, pool2_debug, 3)) {
//			break;
//		}
		
		CONVOLUTION_LAYER_3_SW(pool2,Wconv3,bconv3,hconv3);
//		cout << "CONVOLUTION_LAYER_3_SW :done" << endl;
		CONVOLUTION_LAYER_3_SW_debug(pool2,Wconv3,bconv3,hconv3_debug);
//		cout << "CONVOLUTION_LAYER_3_SW_debug :done" << endl;
//		if(!compare(hconv3, hconv3_debug, 4)) {
//			break;
//		}
		
		FULLY_CONNECTED_LAYER_1_SW(hconv3,Wfc1,bfc1,hfc1);
//		cout << "FULLY_CONNECTED_LAYER_1_SW :done" << endl;
//		FULLY_CONNECTED_LAYER_1_SW_debug(hconv3,Wfc1,bfc1,hfc1_debug);
//		cout << "FULLY_CONNECTED_LAYER_1_SW_debug :done" << endl;
//		compare(hfc1, hfc1_debug, 5);
		
		FULLY_CONNECTED_LAYER_2_SW(hfc1,Wfc2,bfc2,output);
//		cout << "FULLY_CONNECTED_LAYER_2_SW :done" << endl;
//		FULLY_CONNECTED_LAYER_2_SW_debug(hfc1,Wfc2,bfc2,output_debug);
//		cout << "FULLY_CONNECTED_LAYER_2_SW_debug :done" << endl;
//		compare(output, output_debug, 6);
		
//		cout << "TEST" << i << endl;
//		for(int m = 0; m < 10; m++){
//			cout << output[m] << " ";
//		}
//		cout << "Predict:" << argmax(output) << " Actual:" << MNIST_LABEL[i*image_Batch] << endl;
		for(int batch_cnt = 0; batch_cnt < image_Batch; batch_cnt++) {
			result_sw.push_back(equal(MNIST_LABEL[i*image_Batch+batch_cnt],argmax(output + batch_cnt*10)));
		}
